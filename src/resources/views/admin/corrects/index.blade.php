...
@section('content')
    ...
    <!-- 承認待ち・承認済みの切り替え -->
    <div class="tabs">
        <a href="{{ route('admin.corrects.index', ['status' => 'pending']) }}" class="tab {{ request('status', 'pending') == 'pending' ? 'active' : '' }}">承認待ち</a>
        <a href="{{ route('admin.corrects.index', ['status' => 'approved']) }}" class="tab {{ request('status', 'pending') == 'approved' ? 'active' : '' }}">承認済み</a>
    </div>
